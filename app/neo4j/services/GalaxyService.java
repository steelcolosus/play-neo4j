...
import neo4j.relations.RelTypes;
...
public class GalaxyService {
	...
	public List<World> getAllWorlds() {
		return new ArrayList<World>(IteratorUtil.asCollection(worldRepository
				.findAll()));
	}
	...
	public List<World> getWorldPath(final World worldA, final World worldB) {
		Path path = GraphAlgoFactory.shortestPath(
				Traversal.expanderForTypes(RelTypes.REACHABLE_BY_ROCKET,
						Direction.OUTGOING).add(
						RelTypes.REACHABLE_BY_ROCKET), 100)
				.findSinglePath(
						Neo4JServiceProvider.get().template.getNode(worldA.id),
						Neo4JServiceProvider.get().template.getNode(worldB.id));
		if (path == null) {
	...
	public List<World> getByName(String name) {
		...
		List<World> worlds = worldRepository.findByName(name);

		return worlds;
	}

	public List<World> getByMoons(int moons) {

		Logger.debug("parameter: " + moons);
		List<World> worlds = worldRepository.findByMoonsGreaterThan(moons);
	...
	private List<World> convertNodesToWorlds(final Path list) {
		...
		for (Node node : list.nodes()) {
			result.add(Neo4JServiceProvider.get().template.load(node,
					World.class));
		}
		return result;
	}
